...
# =============================================================================
# SCIKIT-LEARN LINEAR REGRESSION IMPLEMENTATION SECTION
# =============================================================================

def sklearn_linear_regression(X, y):
    """
    SCIKIT-LEARN LINEAR REGRESSION WITH BETTER NUMERICAL STABILITY
    """
...
def calculate_r_squared(X, y, slope, intercept):
    """
    CALCULATION OF R-SQUARED FOR CONFIDENCE SCORING USING SCIKIT-LEARN
    """
    try:
        X = np.array(X, dtype=float).reshape(-1, 1)
        y = np.array(y, dtype=float)
...
def predict_pollutants(df_filtered, pollutant_cols, target_date):
    """
    IMPROVED PREDICTION FUNCTION WITH SCIKIT-LEARN FOR BETTER ACCURACY
    """
    predictions = {}
    prediction_notes = {}
    ...
    for col in pollutant_cols:
        try:
            ...
            r2 = calculate_r_squared(X_clean, y_clean, slope, intercept)
            ...
            if pred_val < 0:
                ...
                prediction_notes[col] = "Standard linear regression prediction"
...
def predict_next_7_days(df_filtered, pollutant_cols):
    """
    IMPROVED 7-DAY PREDICTION WITH SCIKIT-LEARN AND BETTER AQI CALCULATION
    """
    next_7_days = []
...
if len(df_filtered) < 3:
    ...
    # Generate 7-day predictions automatically using SCIKIT-LEARN regression
    with st.spinner("🌤️ Generating 7-day forecast using SCIKIT-LEARN LINEAR REGRESSION..."):
...
st.markdown(
    ...
    "<small>Enhanced data processing | All major pollutants | Robust CSV handling</small>"
    "</div>",
    unsafe_allow_html=True
)